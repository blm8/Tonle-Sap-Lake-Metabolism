...
##  Description:  Determines all possible fractional contributions by sources of photoautotrophic and chemoautotrophic C to invertebrates and fishes given either a normal distribution or continuous uniform distribution of values for each source;
...
#?combinations() 
combos <- combinations(length(df$fractions), 6, df$fractions, repeats.allowed = TRUE)
unique(combos[,1]) # Check that all expected values appear in a given column
subset_vctr <- which(Rfast::rowsums(combos[,1:6]) == 1) # Subset rows that sum to 1.00
...
col_order <- c("6", "1", "2", "3", "4", "5") 
result2 <- result[, col_order]
col_order <- c("5", "6", "1", "2", "3", "4")
result3 <- result[, col_order]
col_order <- c("4", "5", "6", "1", "2", "3")
result4 <- result[, col_order]
col_order <- c("3", "4", "5", "6", "1", "2")
result5 <- result[, col_order]
col_order <- c("2", "3", "4", "5", "6", "1")
result6 <- result[, col_order]
colnames(result) <- c("f1", "f2", "f3", "f4", "f5", "f6")
colnames(result2) <- c("f1", "f2", "f3", "f4", "f5", "f6")
colnames(result3) <- c("f1", "f2", "f3", "f4", "f5", "f6")
colnames(result4) <- c("f1", "f2", "f3", "f4", "f5", "f6")
colnames(result5) <- c("f1", "f2", "f3", "f4", "f5", "f6")
colnames(result6) <- c("f1", "f2", "f3", "f4", "f5", "f6")
fractions <- rbind(result, result2, result3, result4, result5, result6)
...
write.csv(fractions,'fractions_0.050_fw.csv')


##===============================  Stable Isotope Mixing Model, Multiple End Members

setwd("~/Desktop/Working")
fractions <- read_csv("fractions_0.050_fw.csv")
fractions <- fractions[,-1]
#View(fractions)

x <- replicate(10000, { # Using Monte Carlo sensitivity analysis
  for (i in 1:dim(fractions)[1]) { # Using n, mean, and sd in a normal distribution for each end member
    #phyto <- -24
    phyto <- rnorm(n=length(fractions$f1), mean=-24, sd=4)
    f_phyto <- fractions$f1
    #peri <- -28
    peri <- rnorm(n=length(fractions$f2), mean=-28, sd=4)
    f_peri <- fractions$f2
    #c3plant <- -29
    c3plant <- rnorm(n=length(fractions$f3), mean=-29, sd=2)
    f_c3plant <- fractions$f3
    macro <- rnorm(n=length(fractions$f4), mean=-33, sd=4)
    f_macro <- fractions$f4
    #c4plant <- -12.2
    c4plant <- rnorm(n=length(fractions$f5), mean=-12.2, sd=0.3)
    f_c4plant <- fractions$f5
    CH4 <- runif(length(fractions$f6), min=-110, max=-50) # Re-populate from either acetate fermentation, or both acetate fermentation and carbonate reduction
    f_CH4 <- fractions$f6
    d13C_calc = (phyto*f_phyto) + (peri*f_peri) + (c3plant*f_c3plant) + (macro*f_macro) + (c4plant*f_c4plant) + (CH4*f_CH4)
    outcome1 <- data.frame(d13C_calc, f_phyto, f_peri, f_c3plant, f_macro, f_c4plant, f_CH4)
    criteria1 <- -27 + 2 # Use +/- 1 per mill, se, or sd
    criteria2 <- -27 - 2
    outcome2 <- outcome1[!(outcome1$d13C_calc>criteria1),]
    outcome3 <- outcome2[!(outcome2$d13C_calc<criteria2),]
...
##===============================  Stable Isotope Mixing Models, Two End Members


hist(runif(length(fractions$f1), min=-37, max=-9.9)) # Run repeatedly to see different continuous uniform distributions possible # Use Monte Carlo sensitivity analysis
...
for (i in 1:dim(fractions)[1]) { # v.3 using n, mean, and sd in a continuous uniform distribution encompassing all end members
  other <- runif(length(fractions$f1), min=-37, max=-9.9)
  ...
  f_CH4 <- fractions$f2
  d13C_calc = (other*f_other) + (CH4*f_CH4)
  outcome1 <- data.frame(d13C_calc, f_other, f_CH4)
...
text(0, 10, expression("C4" ~ "Grasses"), cex=2, font=3, col="gray40")
...
text(-14, 6, expression("C3" ~ "Veg."), cex=2, font=3, col="black")
text(-10, 5, expression("Macrophytes"), cex=2, font=3, col="black")
...
text(0.8, 0.95, expression(italic("Acetate")), cex=2, font=3, col="gray40")
text(0.8, 1.15, expression(italic("Carbonate")), cex=2, font=3, col="gray40")
...
legend("right", inset=0, legend=c("Phyto.", "Peri.", "C3 Veg.", "Macrophytes", "C4 Grasses", "DIC"), 
